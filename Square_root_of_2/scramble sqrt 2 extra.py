...
def scramble_digit_mth_digit_is_2_in_base_sqrt_2(numbers, m, a, b):

    numbers_gpu = cp.asarray(numbers)

    array_1 = increasing_whole_numbers_2(m+1, a, b)
    array = array_1 // 3 ** (m+1 - 3)
    result = np.append(np.argwhere(array == 9),
                       np.argwhere(array == 10), axis=0)
    indicies = cp.asarray(result.reshape(result.shape[0]))
    prob_of_staying_the_same = 10
    random_0_1 = cp.random.randint(
        prob_of_staying_the_same, size=indicies.shape[0], dtype='int32') // (prob_of_staying_the_same-1)
    random_0_1 = cp.zeros(indicies.shape[0], dtype='int32')
    random_0_1 = (1-random_0_1) * 3 ** (m+1 - 2)

    increasing_gpu = cp.asarray(array_1)
    permutation_gpu = cp.copy(increasing_gpu)
    permutation_gpu[indicies] = cp.add(permutation_gpu[indicies], random_0_1)
    indicies += 1
    permutation_gpu[indicies] = cp.add(permutation_gpu[indicies], random_0_1)
    indicies += 1
    permutation_gpu[indicies] = cp.add(permutation_gpu[indicies], random_0_1)
    indicies += 1
    permutation_gpu[indicies] = cp.subtract(
        permutation_gpu[indicies], random_0_1)
    indicies += 1
    permutation_gpu[indicies] = cp.subtract(
        permutation_gpu[indicies], random_0_1)
    indicies += 1
    ...
    arg_sorted_gpu = np.argsort(increasing_gpu)
    permutation_gpu = permutation_gpu[arg_sorted_gpu]
    increasing_gpu = increasing_gpu[arg_sorted_gpu]
    numbers_gpu = permutation_gpu[cp.searchsorted(
        increasing_gpu, numbers_gpu % (a+1)**(m+1))] + (numbers_gpu // (a+1)**(m+1))*(a+1)**(m+1)

    numbers = cp.asnumpy(numbers_gpu)
    return numbers


def scramble_points_sqrt_2_full(point_numbers, num_digits, a, b):
    # for m in range(1, num_digits+1):
    #     point_numbers[:, 0] = scramble_mth_digit_general(
    #         point_numbers[:, 0], m, a, b)
    # for m in range(1, num_digits+1):
    #     point_numbers[:, 0] = scramble_digit_mth_digit_is_2_in_base_sqrt_2(
    #         point_numbers[:, 0], m, a, b)
    # for m in range(1, num_digits+1):
    #     point_numbers[:, 0] = scramble_mth_digit_general(
    #         point_numbers[:, 0], m, a, b)
    ...
    #         point_numbers[:, 1], m, a, b)
    for m in range(1, 2):
        point_numbers[:, 1] = scramble_digit_mth_digit_is_2_in_base_sqrt_2(
            point_numbers[:, 1], m, a, b)
    # for m in range(1, num_digits+1):
    #     point_numbers[:, 1] = scramble_mth_digit_general(
    #         point_numbers[:, 1], m, a, b)
...
def scrambled_hammersley_set_full(num_digits, a, b):
    point_numbers = hammersley_numbers(num_digits, a, b)
    scramble_points_sqrt_2_full(point_numbers, num_digits, a, b)
...
b = 1
# print('Increasing')
# print(hammersley_digits(num_digits, a, b)[:, 0])
# print()
...
# print()
# print(numbers_to_digits(van_der_corput_numbers(num_digits, a, b), num_digits, a))


num_digits = 3
a = 2
b = 1
point_values = hammersley_values_2(num_digits, a, b)

print(point_values.shape[0])
plot_points(point_values, a, b, 2, 0, 15)
plot_points(point_values, a, b, 1, 1, 15)
plot_points(point_values, a, b, 0, 2, 15)

point_values = scrambled_hammersley_set_full(num_digits, a, b)
...
#             point_values, scrambled_hammersley_set_full(num_digits, a, b), axis=0)
