...
# #             a, b, num_digits, num_digits, 10)
# for num_digits in range(2, 6):
#     points = scrambled_hammersley_set(num_digits, a, b)
#     for _ in range(75000 // num_whole_numbers_2(num_digits, a, b) - 1):
#         points = np.append(points, scrambled_hammersley_set(num_digits, a, b), axis=0
#                            )
#     plot_points(points, a, b, num_digits, num_digits, 1)


num_points = num_whole_numbers_2(num_digits, a, b)

# base = 1+math.sqrt(3).real
# print(f'{num_digits} digits gives {num_points} points')
# for m in range(num_digits+3):
#     print(f'gamma^-{m} : {round(num_points * base ** (-m))} : {round(2*num_points*base**(-m-1))} : {round(4*num_points*base**(-m-2))}')
# print()
base = 1+math.sqrt(2).real

num_points_2 = round(base ** num_digits)
num_points_3 = pell_numbers(num_digits)
print(f'{num_digits} digits gives {num_points} points')
print(f'{num_digits} digits gives {num_points_2} points')
print(f'{num_digits} digits gives {num_points_3} points')
print()
for m in range(num_digits+3):
    print(f'gamma^-{m} : {round(num_points * base ** (-m))} : {round(num_points*base**(-m-1))} : {round(num_points*base**(-m-2))}')
    print(f'gamma^-{m} : {round(num_points_2 * base ** (-m))} : {round(num_points_2*base**(-m-1))} : {round(num_points_2*base**(-m-2))}')
    print(f'gamma^-{m} : {round(num_points_3 * base ** (-m))} : {round(num_points_3*base**(-m-1))} : {round(num_points_3*base**(-m-2))}')
    print()


# # print(hammersley_digits(2, 2, 2))
# for n in range(0, num_digits-t+1):
#     plot_points(hammersley_values_2(0, a, b),
#                 a, b, n, num_digits - n - t, 10)
plot_points(hammersley_values_2(0, a, b),
            a, b, 1, 2, 10)
plot_points(hammersley_values_2(0, a, b),
            a, b, 2, 2, 10)
...
# repeats = 500
# star_disc = np.zeros(repeats, dtype='float')
...
#     star_disc[n] = max(result[0, 0], result[1, 0])
#     if n % 25 == 0:
#         print(n)
# total_time = time.perf_counter() - start_time

# pd.DataFrame(star_disc).to_csv('star discrepancy histogram data.csv')
# print(f'{total_time}')
...
# start_time = time.perf_counter()
# for n in range(repeats):
#     points = qmcpy.DigitalNetB2(dimension=2, randomize=True,
#                                 graycode=False).gen_samples(n_min=0, n_max=2**15)
#     result = star_discrepancy_largest_rest(points)
#     star_disc[n] = max(result[0, 0], result[1, 0])
#     if n % 25 == 0:
#         print(n)
...
# pd.DataFrame(star_disc).to_csv('star discrepancy histogram data base 2.csv')
# print(f'{total_time}')
# print(points.shape[0])
