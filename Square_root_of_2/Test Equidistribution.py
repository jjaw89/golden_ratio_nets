...
discrepancies = ctypes.CDLL(
    'c:/Users/Jaspar/Dropbox/PythonProjects/Square_root_of_2/discrepancies.dll')

# Define the return type and argument types for the function


class Results(ctypes.Structure):
    _fields_ = [("max_closed", ctypes.c_double),
                ("max_closed_right", ctypes.c_double),
                ("max_closed_top", ctypes.c_double),
                ("max_open", ctypes.c_double),
                ("max_open_right", ctypes.c_double),
                ("max_open_top", ctypes.c_double)]


# Set the argument types
discrepancies.star_discrepancy_largest_rest.argtypes = [
    ctypes.POINTER(ctypes.POINTER(ctypes.c_double)), ctypes.c_int]
discrepancies.star_discrepancy_largest_rest.restype = Results

# Memory management functions
discrepancies.create_2D_array.argtypes = [ctypes.c_int, ctypes.c_int]
discrepancies.create_2D_array.restype = ctypes.POINTER(
    ctypes.POINTER(ctypes.c_double))

discrepancies.free_2D_array.argtypes = [ctypes.POINTER(
    ctypes.POINTER(ctypes.c_double)), ctypes.c_int]


def call_discrepancy(points):
    n = len(points)
    c_array = discrepancies.create_2D_array(n, 2)

    for i in range(n):
        for j in range(2):
            c_array[i][j] = points[i][j]

    result = discrepancies.star_discrepancy_largest_rest(c_array, n)

    discrepancies.free_2D_array(c_array, n)
...
# points = [(0.1, 0.2), (0.3, 0.7), (0.5, 0.6), (0.9, 0.4)]
# result = call_discrepancy(points)

# print(result.max_closed, result.max_closed_right, result.max_closed_top)


# # Your variables
# a = 2  # Just an example value, replace with actual value
# b = 1  # Just an example value, replace with actual value
# num_digits = 10  # Just an example value, replace with actual value
...
points = hammersley_values(num_digits, a, b)
start_time = time.perf_counter()
star_discrepancy_largest_rest_optimized(points)
end_time = time.perf_counter()
print(
    f'num_digits={num_digits}, Calculation took {end_time-start_time} seconds.')
points = hammersley_values(num_digits, a, b)
...
points = hammersley_values(num_digits, a, b)
start_time = time.perf_counter()
result = call_discrepancy(points, points.shape[0])
