...
@njit()
def negative_base_van_der_corput_values(num_digits, a, b):
    base = ((-math.sqrt(a ** 2+4*b)+a) / 2).real
    print(base**2)
    vdc_numbers = van_der_corput_numbers(num_digits, a, b)
    vdc_values = np.zeros_like(vdc_numbers, dtype='float')
    for n in range(vdc_numbers.shape[0]):
        number = vdc_numbers[n]
        for j in range(num_digits):
            vdc_values[n] += (number % (a+1)) * base**(j+1)
...
def new_partition(num_digits, a, b):
    return negative_base_van_der_corput_values(num_digits, a, b)


def neg_base_hammersley(num_digits, a, b):
    x_values = negative_base_van_der_corput_values(num_digits, a, b)
    y_values = increasing_whole_numbers_values_2(num_digits, a, b)
    return np.array([[x_values[n], y_values[n]] for n in range(x_values.shape[0])], dtype='float')


def plot_points_neg(points, a, b, x_partition, ypartition, size):
    x = points[:, 0]
    y = points[:, 1]
    plt.figure(figsize=(12, 12))
    ax = plt.gca()
    ...
    plt.plot(x, y, '.', markersize=size)
    plt.grid()
    plt.xticks(ticks=new_partition(
        x_partition, a, b), labels=[])
    plt.yticks(ticks=increasing_whole_numbers_values_2(
        ypartition, a, b), labels=[])
    plt.xlim((0, 1))
...
def denominator(n):
    if n == 0:
        ...
        current = 1
        for _ in range(n-1):
            minus_0 = minus_2+2*minus_1
            minus_2 = minus_1
            minus_1 = minus_0
        return minus_0

# print(new_partition(2, a, b))
# print()


# print(hammersley_digits(4, 2, 1))
for n in range(1, 4):
    # print(f'{round((1+math.sqrt(2))**-3*num_whole_numbers_2(n, 2, 1))} / {num_whole_numbers_2(n, 2, 1)}')
    # print(f'{round((1+math.sqrt(2))**-1*denominator(n))} / {denominator(n)}')
    plot_points(hammersley_values(0, 2, 1), 2, 1, n, 0, 1)
# plot_points_neg(neg_base_hammersley(num_digits, a, b), a, b, 2, 0, 10)

# plot_points(hammersley_values(3, a, b), a, b, 1, 1, 10)
# plot_points(hammersley_values(3, a, b), a, b, 2, 1, 10)
# plot_points(hammersley_values(3, a, b), a, b, 1, 2, 10)
# plot_points(hammersley_values(3, a, b), a, b, 2, 2, 10)
