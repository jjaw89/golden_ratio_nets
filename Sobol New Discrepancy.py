...
# sobol = qmcpy.Sobol(2)
# points = sobol.gen_samples(2**21)

# print(points.shape)


@njit()
def generate_base_2_hammersley(num_digits):
    numbers = np.arange(0, 2**num_digits, 1)
    # print(numbers)
    points = np.zeros((2**num_digits, 2), dtype='float')

    for m in range(1, num_digits+1):
        digits = numbers % 2
        points[:, 0] += digits * .5**m
        points[:, 1] += digits * .5 ** (num_digits+1-m)
        numbers //= 2
        # print(numbers)
    return points
...
for m in range(1, 10):
    points = convert_integer_points_to_decimals(generate_hammersley_numbers(m))
    start_time = time.perf_counter()
    l2 = l2_star_discrepancy_cuda(points)
    # star = star_discrepancy_largest_rest(points)[0, 0]
    end_time = time.perf_counter()
    print(f'{end_time-start_time}, {m}, {points.shape[0]}, {l2}')
