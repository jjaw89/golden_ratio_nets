...
def digits_in_common(int_1, int_2):
    arr = np.sort(np.array([integer_to_decimal(int_1),
                            integer_to_decimal(int_2)]))
    # print(arr)
    multiplied = arr[1]-arr[0]
    # print(multiplied)
    for j in range(32):
        multiplied = multiplied * GOLDEN_RATIO
        if multiplied + GOLDEN_RATIO ** (-34) >= 1:
...
# @njit(parallel=True)
def make_table(num_digits):
    digits_in_common_2 = np.zeros(
    ...
    for n in prange(FIBONACCI[num_digits+2]):
        for m in range(n, FIBONACCI[num_digits+2]):
            digits_in_common_2[n, m] = digits_in_common_log(
                numbers[n], numbers[m])
            digits_in_common_2[m, n] = digits_in_common_2[n, m]
    return digits_in_common_2


# @njit()
def make_table_digits_in_common_cp(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = convert_integers_to_decimals(numbers)
    decimal_table = cp.absolute(
        cp.array([decimals - x for x in decimals],
                 dtype='float16'))+GOLDEN_RATIO ** (-34)
    decimal_table = -cp.log2(decimal_table) / LOG_GOLDEN_RATIO
    decimal_table = decimal_table.astype('int8')
    return cp.asnumpy(decimal_table)


def make_table_digits_in_common_cp_3(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = convert_integers_to_decimals(numbers)
    decimal_table = cp.absolute(
        cp.array([decimals - x for x in decimals],
                 dtype='f4'))+GOLDEN_RATIO ** (-34)
    decimal_table = -cp.log2(decimal_table) / LOG_GOLDEN_RATIO
    decimal_table = decimal_table.astype('int8')
    return cp.asnumpy(decimal_table)


def make_table_digits_in_common_32_cp():
    numbers = no_consecutive_ones_binary_2(32)
    decimals = convert_integers_to_decimals(numbers)
    decimal_table = cp.absolute(
        cp.array([decimals - decimals[n] for n in range(25)],
                 dtype='f4'))+GOLDEN_RATIO ** (-34)
...
def make_table_digits_in_common_32():
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = np.array([integer_to_decimal(n) for n in numbers], dtype='f4')
    decimal_table = np.zeros(
        (FIBONACCI[num_digits+2], FIBONACCI[num_digits+2]), dtype='f4')
    for n in range(FIBONACCI[num_digits+2]):
        decimal_table[n] = decimals - decimals[n]
    decimal_table = np.array([decimals - decimals[n]
                             for n in range(25)], dtype='f4')
    decimal_table = np.absolute(decimal_table)
    decimal_table += GOLDEN_RATIO ** (-34)
    decimal_table = - np.log2(decimal_table) / LOG_GOLDEN_RATIO
    decimal_table = decimal_table.astype('int8')
    return decimal_table


@njit()
def make_table_digits_in_common_numba(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = np.array([integer_to_decimal(n) for n in numbers], dtype='f4')
    decimal_table = np.zeros(
        (FIBONACCI[num_digits+2], FIBONACCI[num_digits+2]), dtype='f4')
...
# @njit(parallel=True)
# def make_table_digits_in_common_numpy_v2(num_digits):
#     numbers = no_consecutive_ones_binary_2(num_digits)
#     decimals = np.array([integer_to_decimal(n)
#                         for n in numbers], dtype='f4')
#     results_table = np.zeros(
#         (FIBONACCI[num_digits+2], FIBONACCI[num_digits+2]), dtype='int8')
#     for n in prange(FIBONACCI[num_digits+2]-1):
#         decimal_table = decimals[n+1:] - decimals[n]
#         # decimal_table = np.array([decimals - x for x in decimals], dtype='f4')
#         decimal_table = np.absolute(decimal_table)
#         decimal_table += GOLDEN_RATIO ** (-34)
#         decimal_table = - np.log2(decimal_table) / LOG_GOLDEN_RATIO
#         results_table[n, n+1:] = np.ceil(decimal_table)//1
...
def make_table_digits_in_common_numpy_v2(num_digits):
    ...
    results_table = np.zeros(
        (FIBONACCI[num_digits+2], FIBONACCI[num_digits+2]), dtype='int8')
    for n in prange(FIBONACCI[num_digits+2]):
        decimal_table = decimals - decimals[n]
        # decimal_table = np.array([decimals - x for x in decimals], dtype='f4')
        decimal_table = np.absolute(decimal_table)
        decimal_table += GOLDEN_RATIO ** (-34)
        decimal_table = - np.log2(decimal_table) / LOG_GOLDEN_RATIO
        results_table[n] = np.ceil(decimal_table)//1
    for n in prange(FIBONACCI[num_digits+2]):
        results_table[n, n] = 0
    return results_table
...
@njit(parallel=True)
def make_table_digits_in_common_numpy_v3(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = np.array([integer_to_decimal(n)
                        for n in numbers], dtype='f4')
    results_table = np.zeros(
...
@njit(parallel=True)
def make_table_digits_in_common_numpy_v4(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = np.array([integer_to_decimal(n)
                        for n in numbers], dtype='f4')
    results_table = np.zeros(
    ...
    for n in prange(FIBONACCI[num_digits+2]):
        for k in range(n+1, FIBONACCI[num_digits+2]):
            ...
            # decimal_table = np.array([decimals - x for x in decimals], dtype='f4')
            decimal = abs(decimal)
            decimal += GOLDEN_RATIO ** (-34)
            decimal = - math.log(decimal) / LOG_GOLDEN_RATIO_2
            results_table[n, k] = math.ceil(decimal) // 1
    for n in prange(1, FIBONACCI[num_digits+2]):
        for k in range(n):
            results_table[n, k] = results_table[k, n]
    return results_table
...
@njit(parallel=True)
def make_table_digits_in_common_numpy_v5(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = np.array([integer_to_decimal(n)
                        for n in numbers], dtype='f4')
    results_table = np.zeros(
...
def make_table_digits_in_common_cp_v2(num_digits):
    numbers = no_consecutive_ones_binary_2(num_digits)
    decimals = cp.array([integer_to_decimal(n)
                        for n in numbers], dtype='float')
    results_table = cp.zeros(
        (FIBONACCI[num_digits+2], FIBONACCI[num_digits+2]), dtype='int8')
    for n in range(FIBONACCI[num_digits+2]-1):
        decimal_table = decimals[n+1:] - decimals[n]
        # decimal_table = np.array([decimals - x for x in decimals], dtype='f4')
        decimal_table = cp.absolute(decimal_table)
        decimal_table += GOLDEN_RATIO ** (-34)
        decimal_table = - cp.log2(decimal_table) / LOG_GOLDEN_RATIO
        results_table[n, n+1:] = cp.ceil(decimal_table)//1
    return cp.asnumpy(results_table)


# make_table_digits_in_common_numpy(5)
# total_start_time = time.perf_counter()
# table_1 = make_table_digits_in_common_numpy(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Time Numpy : {total_end_time-total_start_time:0.6f}")
# print(table_1)

# make_table_digits_in_common_numpy_v2(5)
# total_start_time = time.perf_counter()
# # table_1 = np.byte(np.ceil(make_table_digits_in_common_numpy_v2(num_digits)))
# table_1 = make_table_digits_in_common_numpy_v2(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Time Numba v2: {total_end_time-total_start_time:0.6f}")
# print(table_1)
# # print(table_1.nbytes / (8 * 10 ** 9))

# make_table_digits_in_common_numpy_v3(5)
# total_start_time = time.perf_counter()
# # table_1 = np.byte(np.ceil(make_table_digits_in_common_numpy_v2(num_digits)))
# table_1_1 = make_table_digits_in_common_numpy_v3(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Time Numba v3: {total_end_time-total_start_time:0.6f}")
# print(table_1_1)
num_digits = 23

# make_table_digits_in_common_numpy_v4(5)
# total_start_time = time.perf_counter()
# # table_1 = np.byte(np.ceil(make_table_digits_in_common_numpy_v2(num_digits)))
# table_1_2 = make_table_digits_in_common_numpy_v4(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Time Numba v4: {total_end_time-total_start_time:0.6f}")
# print(table_1_2)

######################################
# make_table_digits_in_common_numpy_v5(5)
# total_start_time = time.perf_counter()
# # table_1 = np.byte(np.ceil(make_table_digits_in_common_numpy_v2(num_digits)))
# table_1_3 = make_table_digits_in_common_numpy_v5(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Time Numba v5: {total_end_time-total_start_time:0.6f}")
# print(table_1_3.shape)
...
total_start_time = time.perf_counter()
table = np.load(path/'digits_in_common_23.npy')
total_end_time = time.perf_counter()
print(
    f"Time Numba v5: {total_end_time-total_start_time:0.6f}")
# print(table.shape)
# make_table_digits_in_common_cp_v2(5)
# total_start_time = time.perf_counter()
# table_2 = make_table_digits_in_common_cp_v2(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Time CP v2: {total_end_time-total_start_time:0.6f}")
# print(table_2)

# print(np.array_equal(table_1_2, table_1_3))


# make_table_digits_in_common_cp(5)
# total_start_time = time.perf_counter()
# table_2 = make_table_digits_in_common_cp(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Total Cupy : {total_end_time-total_start_time:0.6f}")
# print(table_2)

# make_table_digits_in_common_cp(5)
# total_start_time = time.perf_counter()
# table_3= make_table_digits_in_common_cp_3(num_digits)
# total_end_time = time.perf_counter()
# print(
#     f"Total Cupy 3: {total_end_time-total_start_time:0.6f}")
# print(table_3)
...
# decimals = convert_integers_to_decimals(numbers)
# decimal_table = np.array([decimals - decimals[n]
#                          for n in range(200)], dtype='f4')
# print(decimal_table.nbytes / (8 * 10 ** 9))
