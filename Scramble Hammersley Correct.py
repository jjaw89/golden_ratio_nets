...
def correct_scramble_hammersley(num_digits):
    ...
    # scramble last digit
    point_numbers[:, 0] = scramble_numbers_m_digits(
        num_digits, point_numbers[:, 0])
    point_numbers[:, 1] = scramble_numbers_m_digits(
        num_digits, point_numbers[:, 1])

    points = convert_to_points_and_scramble_tail_end(point_numbers)
    # y_values = convert_to_points_and_scramble_tail_end(point_numbers[:, 1])

    return points


def generate_n_scrambled_hammersley(n, num_digits):
    # k = num_digits
    # point_numbers_1 = generate_random_sequence(k)
    points = correct_scramble_hammersley(num_digits)
    # print(point_numbers.shape)
    # print(FIBONACCI[num_digits+2])
    # print(generate_random_sequence(num_digits).shape)

    for k in range(n):
        temp_points = correct_scramble_hammersley(num_digits)
...
# plot_points(generate_n_scrambled_hammersley(1000, 5), 5, 5)
...
#         point_set = generate_n_scrambled_hammersley(num_digits)
...
#     for num_digits in range(1, 22):  # 22
#         point_set = point_set = correct_scramble_hammersley(num_digits)
...
for num_digits in range(5, 25):

    num_repeats = 20000 // FIBONACCI[num_digits+2]
    start_time = time.perf_counter()
    points = correct_scramble_hammersley(num_digits)
    ...
    for n in range(num_repeats):
        ...
        points_temp = correct_scramble_hammersley(num_digits)
    ...
    print(
        f'Scrambled Hammersley {num_digits} digits : {points.shape[0]} points')
