...
values = sum_arr[FIBONACCI[m+2]:FIBONACCI[m+3]]

test_values = np.array([solved_recurrence(m) + sum_arr[n-FIBONACCI[m+2]] - (n-FIBONACCI[m+2]+1)/GOLDEN_RATIO**(m+1)
                       for n in range(FIBONACCI[m+2], FIBONACCI[m+3])], dtype='float')

# test_values_2 = np.array([1/GOLDEN_RATIO**(n)
#                           for n in range(5)], dtype='float')

# start = sum_arr[:FIBONACCI[m+1]]
# start[0] = 0
# print(values)
# print(test_values)
# print(sum_arr[FIBONACCI[m+2]]-solved_recurrence(m)-.5)
# print(GOLDEN_RATIO**(-m-1))
# print(values - test_values)

vector_results = []
j = 8
for n in range(FIBONACCI[num_digits]):
    vector_results += [sum_arr[n+FIBONACCI[j]]-sum_arr[n+FIBONACCI[j-1]]]
arr = np.array(vector_results)
# print(arr)
pd.DataFrame(arr).to_csv('Harr sums.csv')


# c = 2
# log_array = np.array(
#     [c*math.log(FIBONACCI[m+2]+n).real for n in range(FIBONACCI[m+1])])
# print(log_array - values)

# for m in range(10):
#     print(solved_recurrence(m))
