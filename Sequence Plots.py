...
for _ in range(12):  # 26 for starting with 2
    start_time = time.perf_counter()
...
def plot_points_2(points, num_digits, x_partition, ypartition):
    ...
    plt.plot(points[:fibonacci(num_digits+1), 0],
             points[:fibonacci(num_digits+1), 1], 'o', markersize=12, c='black')
    plt.plot(points[fibonacci(num_digits+1):fibonacci(num_digits+2), 0],
             points[fibonacci(num_digits+1):fibonacci(num_digits+2), 1], 'o', markersize=12, c='black')
    plt.plot(points[fibonacci(num_digits+1):fibonacci(num_digits+2), 0],
             points[fibonacci(num_digits+1):fibonacci(num_digits+2), 1], 'o', markersize=9, c='white')
    plt.grid()
    plt.xticks(ticks=partition(x_partition), labels=[])
    plt.yticks(ticks=partition(ypartition), labels=[])
    plt.xlim((0, 1))
...
for m in range(1, 13):
    plot_points_2(points[:FIBONACCI[m+2]], m, 0, 0)
    # plot_points(points[:FIBONACCI[m+2]], 0, 0, 12)
