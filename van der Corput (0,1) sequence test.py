...
def check_equidistribution_njit(x_numbers, y_numbers, num_point_digits, num_x_partition_digits, num_y_partition_digits):
    ...
    counting_matrix = partition_numbers_2d_binary(
        num_point_digits, num_x_partition_digits)
...
@njit()
def check_equidistribution_1d(x_numbers, num_point_digits, num_x_partition_digits):
    x_partition = no_consecutive_ones_binary_flipped(num_x_partition_digits)
    x_look_up = np.zeros(
        (2**num_x_partition_digits), dtype='int64')

    for n in range(x_partition.shape[0]):
        x_look_up[x_partition[n]] = n

    counting_matrix = partition_numbers_1d_binary(
        num_point_digits, num_x_partition_digits)

    for n in range(x_numbers.shape[0]):
        counting_matrix[x_look_up[x_numbers[n] %
                                  2**(num_x_partition_digits)]] -= 1
    return counting_matrix
...
@njit()
def is_equal(digits_as_numbers, test_num, power):
    true_list = [0]
    for n in range(digits_as_numbers.shape[0]):
        if digits_as_numbers[n] // 2 ** power == test_num:
            true_list += [digits_as_numbers[n]]
    return np.array(true_list[1:], dtype='int64')


@njit()
def testing(numbers, tail_end_power):
    uniques = np.unique(numbers // 2 ** tail_end_power)
    # print(uniques)
    is_equal(numbers, uniques[2], tail_end_power)

    blocks = [[is_equal(numbers, uniques[0], tail_end_power)]]
    for test in uniques[1:]:
        blocks += [[is_equal(numbers, test, tail_end_power)]]

    # print(blocks)
    # print(numbers)
    for block in blocks:
        # print()
        block = block[0] % 2 ** tail_end_power
        # print(block)
        num_digits_in_block = np.where(FIBONACCI == block.shape[0])[0][0] - 2
        # print(f'block size : {block.shape[0]} fibonacci : {num_digits_in_block}')
        for partition_test_digit_number in range(1, num_digits_in_block-1):
            # partition_test = check_equidistribution_1d(
            #     block[0], np.where(FIBONACCI == 13)[0][0] - 2, 3)
            partition_test = check_equidistribution_1d(
                block, num_digits_in_block, 3)

            # print(
            #     f'partition test number : {partition_test_digit_number}, partition test : {partition_test}')
            # print(no_consecutive_ones_binary_flipped(partition_test_digit_number))
            # print(partition_numbers_1d_binary(
            #     num_digits_in_block, partition_test_digit_number))
            truth = np.array_equal(
                partition_test, np.zeros_like(partition_test, dtype='int'))
            if not truth:
                print('False')


def testing_1(num_digits):
    numbs = no_consecutive_ones_binary(num_digits)

    for n in prange(1, num_digits+1):
        testing(numbs, n)
