...
# start_time = time.perf_counter()
# scramble_non_zero_digits_no_back_tracking(point_numbers)
...
# start_time = time.perf_counter()
# scramble_non_zero_digits_no_back_tracking_cp(point_numbers, 32)
# total_time = time.perf_counter() - start_time
# print(f'cp time {total_time}')
# print()
...
# points = generate_random_sequence_with_scramble(10)

# start_time = time.perf_counter()
# points = generate_random_sequence_with_scramble(21)
# total_time = time.perf_counter() - start_time
# print(f'Time {total_time}')

# plot_points(points, 0, 0, 1)
# for num_digits in range(3, 17):
2
#     num_repeats = 250000 // FIBONACCI[num_digits+2]
#     start_time = time.perf_counter()
#     points = generate_random_sequence_with_scramble(num_digits)
#     # print(f'num_points {points.shape} : {FIBONACCI[num_digits + 2]}')
#     add_points = np.copy(points[FIBONACCI[num_digits+1]:])

#     for n in range(num_repeats):

#         points_temp = generate_random_sequence_with_scramble(num_digits)

#         add_points_temp = points_temp[FIBONACCI[num_digits+1]:]
#         points = np.append(points, points_temp, axis=0)
#         add_points = np.append(add_points, np.copy(add_points_temp), axis=0)
#     end_time = time.perf_counter() - start_time
#     print(f'{num_digits} : {num_repeats} : Time {end_time}')
#     print(points.shape)
#     print(add_points.shape)
#     print(num_repeats * FIBONACCI[num_digits])
#     print(f'sequence with {num_digits} digits : {points.shape[0]} points')
#     plot_points_size_1(points, 0, 0)
#     print(f'{add_points.shape[0]} add points')
#     plot_points_size_1(add_points, 0, 0)


# sequence_numbers = generate_random_sequence(25)

# num_digits = 5
# # print(sequence_numbers[:FIBONACCI[num_digits+2]])
# # find_t_value(sequence_numbers[:FIBONACCI[num_digits+2], :], num_digits)
# for num_digits in range(1, 25):
#     print(
#         f'random sequence: num digits {num_digits} : t-value is {find_t_value(sequence_numbers[:FIBONACCI[num_digits+2]],num_digits)}')

# sequence_numbers = scramble_non_zero_digits_no_back_tracking_cp(
#     sequence_numbers, 30)
# for num_digits in range(1, 25):
#     print(
#         f'scrambled sequence: num digits {num_digits} : t-value is {find_t_value(sequence_numbers[:FIBONACCI[num_digits+2]],num_digits)}')

# for num_digits in range(1, 20):
#     sequence_numbers = scramble_non_zero_digits_no_back_tracking(
#         generate_hammersley_numbers(num_digits))
#     print(
#         f'scrambled hammersley: num digits {num_digits} : t-value is {find_t_value(sequence_numbers[:FIBONACCI[num_digits+2]],num_digits)}')

# for num_digits in range(1, 20):
#     sequence_numbers = scramble_non_zero_digits_no_back_tracking_cp(
#         generate_hammersley_numbers(num_digits), 25)
#     print(
#         f'scrambled hammersley cp: num digits {num_digits} : t-value is {find_t_value(sequence_numbers[:FIBONACCI[num_digits+2]],num_digits)}')
# num_digits = 20
# point_numbers = generate_hammersley_numbers(num_digits)
# for n in range(2):

#     print(
#         f'multiple scrambles: num scrambles {n} : t-value is {find_t_value(point_numbers,num_digits)}')
#     plot_points(convert_integer_points_to_decimals(point_numbers), 0, 0, 3)
#     point_numbers = scramble_non_zero_digits_no_back_tracking_cp(
#         point_numbers, 30)

# num_digits = 6
# point_numbers = scramble_non_zero_digits_no_back_tracking(
#     generate_hammersley_numbers(num_digits))
# print(
#     f'num digits {num_digits} : t-value is {find_t_value(sequence_numbers[:FIBONACCI[num_digits+2]],num_digits)}')

# points = convert_integer_points_to_decimals(point_numbers)
# for s in range(num_digits-2, num_digits+1):
#     for k in range(s+1):
#         print(f't-value is {num_digits - s} :  s={s}, k={k}, s-k={s-k}')
#         plot_points(points[:FIBONACCI[num_digits+2]], k, s-k, 5)
#     # print(partition(k))


# num_digits = 5
# point_numbers = generate_hammersley_numbers(num_digits)
# print(f't-value is {find_t_value(point_numbers,num_digits)}')
# # point_numbers[:, 0] = scramble_numbers_m_digits(4,
# #                                                 point_numbers[:, 0])
# scramble_non_zero_digits_no_back_tracking(point_numbers)
# print(f't-value is {find_t_value(point_numbers,num_digits)}')
# for k in range(num_digits-find_t_value(point_numbers, num_digits)):
#     plot_points(convert_integer_points_to_decimals(
#         point_numbers), k, num_digits - k+1, 10)

# print(convert_numbers_to_digits(
#     no_consecutive_ones_binary_2(num_digits), num_digits))

# num_digits = 6
# numbers = cp.asarray(no_consecutive_ones_binary_flipped(num_digits))
# cp.random.seed(seed=1122)
# for digit in range(1, num_digits):
#     numbers = scramble_numbers_m_digits_cp(digit, numbers)
# point_numbers = generate_hammersley_numbers(num_digits)
# point_numbers[:, 0] = cp.asnumpy(numbers)
# print(find_t_value(point_numbers, num_digits))

# plot_points(convert_integer_points_to_decimals(point_numbers), 0, 0, 5)

# for k in range(num_digits):
#     plot_points(convert_integer_points_to_decimals(
#         point_numbers), k, num_digits - k, 10)
...
# point_numbers = generate_hammersley_numbers(num_digits)
# print(find_t_value(point_numbers, num_digits))
# # plot_points(convert_integer_points_to_decimals(point_numbers), 0, 0, 10)
# cp.random.seed(seed=11122)

# numbers = cp.asarray(point_numbers[:, 0])
# for digit in range(1, num_digits):
#     numbers = scramble_numbers_m_digits_cp(digit, numbers)
# point_numbers[:, 0] = cp.asnumpy(numbers)
# print(find_t_value(point_numbers, num_digits))
...
# points_original = np.copy(point_numbers)
...
# plot_points(convert_integer_points_to_decimals(
#     points_original), 2, 3, 10)
# plot_points(convert_integer_points_to_decimals(
#     point_numbers), 2, 3, 10)
# plot_points(convert_integer_points_to_decimals(
#     points_original), 3, 3, 10)
# plot_points(convert_integer_points_to_decimals(
#     point_numbers), 3, 3, 10)
# # point_numbers[:, 0] = cp.asnumpy(numbers)
# print(find_t_value(point_numbers, num_digits))
# # plot_points(convert_integer_points_to_decimals(point_numbers), 0, 0, 5)

# # for k in range(num_digits+1-find_t_value(point_numbers, num_digits)):
# #     plot_points(convert_integer_points_to_decimals(
# #         points_original), k, num_digits - k-find_t_value(point_numbers, num_digits)+1, 10)

# # for k in range(num_digits-find_t_value(point_numbers, num_digits)+1):
# #     print(f'{k},{num_digits - k-find_t_value(point_numbers, num_digits)+1}')
# #     plot_points(convert_integer_points_to_decimals(
# #         points_original), k, num_digits - k-find_t_value(point_numbers, num_digits)+1, 10)
# #     print('**')
# #     plot_points(convert_integer_points_to_decimals(
# #         point_numbers), k, num_digits - k-find_t_value(point_numbers, num_digits)+1, 10)

# num_digits = 6

# point_numbers = generate_hammersley_numbers(num_digits)
# points = convert_integer_points_to_decimals(point_numbers)
# plot_points(points, 4, 0, 10)
# points[:, 0] *= GOLDEN_RATIO
# plot_points(points, 4, 0, 10)

print(version_3_l2_star_discrepancy(
    generate_random_sequence_with_scramble_cp(10)))
