...
def scramble_numbers_m_digits(m, numbers):
    ...
    # random_zeros_and_ones = np.zeros(FIBONACCI[m+1], dtype='int64')
    random_zeros_and_ones = rand_vect(FIBONACCI[m+1])
...
def scramble_numbers_m_digits_cp(m, numbers):
    m = m-1
    numbers_to_split = cp.array(no_consecutive_ones_binary_2(m).tolist())
    # print(numbers_to_split)
    # print(convert_numbers_to_digits(numbers_to_split, m))

    random_zeros_and_ones = cp.random.randint(
        2, size=FIBONACCI[m+1], dtype='int64')

    # random_zeros_and_ones = cp.array(rand_vect(FIBONACCI[m+1]).tolist())
    # print(random_zeros_and_ones)
    # print(1 - random_zeros_and_ones)
    permutation = cp.arange(FIBONACCI[m+3]) * 0
    permutation[FIBONACCI[m+2]:] = numbers_to_split[:FIBONACCI[m+1]]
    permutation[:FIBONACCI[m+1]] += random_zeros_and_ones * 2**m
    permutation[FIBONACCI[m+2]:] += (1-random_zeros_and_ones) * 2**m
    original_order = cp.array(no_consecutive_ones_binary_2(m+1).tolist())
    # print(original_order)
    # print(permutation)
    # print('original numbers:')
    # print(numbers)
    # print(convert_numbers_to_digits(permutation, m+1))
    look_up_array = cp.arange(original_order[-1]+1) * 0
    look_up_array[original_order] = permutation
    # print(look_up_array)
    # print('first m+1 digits:')
    # print(numbers % 2**(m+1))
    digits_to_permute = numbers % 2**(m+1)
    permuted_digits = cp.array([look_up_array[n] for n in digits_to_permute])
...
def scramble_numbers_cp(numbers_1, up_to):
    numbers_2 = numbers_1.tolist()
    numbers = cp.array(numbers_2)
    for m in range(1, up_to):
        numbers[:, 0] = scramble_numbers_m_digits_cp(m, numbers[:, 0])
    for m in range(1, up_to):
        numbers[:, 1] = scramble_numbers_m_digits_cp(m, numbers[:, 1])
    return numbers


@njit()
def scramble_numbers_2(numbers, up_to):
    numbers[:, 0] = scramble_numbers_m_digits(1, numbers[:, 0])
    for m in range(2, up_to):
        numbers[:, 0] = scramble_numbers_m_digits(m, numbers[:, 0])
        numbers[:, 0] = scramble_numbers_m_digits(m-1, numbers[:, 0])

    numbers[:, 1] = scramble_numbers_m_digits(1, numbers[:, 1])
    for m in range(2, up_to):
        numbers[:, 1] = scramble_numbers_m_digits(m, numbers[:, 1])
        numbers[:, 1] = scramble_numbers_m_digits(m-1, numbers[:, 1])
    return numbers


@njit(parallel=True)
def scramble_single(n):
    scrambling = np.zeros((n*2, 2), dtype='int64')
    for k in prange(n):
        scrambling[2*k:2*k +
                   2] = scramble_numbers(np.array([[0, 0], [1, 1]], dtype='int64'), 30)
...
# # plot_points(point_values, 3, 4)

# # point_values = convert_integer_points_to_decimals(point_numbers)
# # plot_points(point_values, 0, 0)


# scramble_numbers_m_digits(6, x_numbers)


# point_numbers = scramble_numbers_2(np.array([[1, 1]], dtype='int64'), 25)
# point_numbers = np.zeros((10000, 2), dtype='int64')
# point_numbers = np.array([[0, 0], [1, 1]], dtype='int64')

# scramble_numbers(point_numbers, 30)

# print(scramble_single(2))

# scramble_single([[0, 0]], 10)

# for _ in range(13):
#     point_numbers = np.append(
#         point_numbers, scramble_numbers_2(np.copy(point_numbers), 25), axis=0)

# point_values = convert_integer_points_to_decimals(point_numbers)
# plot_points(point_values, 1, 1)

# total_start_time = time.perf_counter()

# points_n = scramble_single(1000)

# total_end_time = time.perf_counter()
# print(f'Total_time : {total_end_time-total_start_time:0.6f}')


# point_values = convert_integer_points_to_decimals(points_n)
# plot_points(point_values, 0, 0)
...
scrambled_points_numbers = scramble_numbers(np.copy(point_numbers), 30)
...
# total_start_time = time.perf_counter()
# scrambled_points_numbers = scramble_numbers_cp(np.copy(point_numbers), 30)
# total_end_time = time.perf_counter()
# print(f'Total_time : {total_end_time-total_start_time:0.6f}')

# for _ in range(200):
#     scrambled_points_numbers = np.append(
#         scrambled_points_numbers, scramble_numbers(np.copy(point_numbers), 30), axis=0)
# # print(scrambled_points_numbers)

# point_set = convert_integer_points_to_decimals(scrambled_points_numbers)

# plot_points(point_set, 0, 0)

# vector_results_1 = []
# vector_results_2 = []
# for num_digits in range(1, 22):
...
#         point_numbers = scramble_numbers_2(point_numbers, 30)
#         point_numbers = scramble_numbers_2(point_numbers, 30)
#         point_numbers = scramble_numbers_2(point_numbers, 30)
...
# total_start_time = time.perf_counter()
# for _ in range(25):  # 26 for starting with 2
#     start_time = time.perf_counter()
...
# point_numbers = scramble_numbers(point_numbers, 8)
# print(convert_point_numbers_to_digits(point_numbers[:FIBONACCI[8]], 8))


# vector_results_1 = []
# vector_results_2 = []
# for num_digits in range(3, 24):
#     start_time = time.perf_counter()

#     result_1 = [FIBONACCI[num_digits]]
#     result_2 = [FIBONACCI[num_digits]]
#     for _ in range(10):

#         point_numbers_2 = np.copy(
#             point_numbers[FIBONACCI[num_digits+1]:FIBONACCI[num_digits+2]])
#         point_numbers_2 = scramble_numbers(point_numbers_2, 30)

#         point_set = convert_integer_points_to_decimals(point_numbers_2)
#         point_set = point_set[point_set[:, 0].argsort()]
#         result_1 += [((version_3_l2_star_discrepancy(point_set))**.5).real]
#         result_2 += [star_discrepancy_parallel(point_set)]
#     result_1 += [point_set.shape[0]]
#     vector_results_1 += [result_1]
#     vector_results_2 += [result_2]
#     mid_time = time.perf_counter() - start_time
#     # [num_digits, point_set.size[0], l2_star_discrepancy(point_set)]
...
#         point_numbers_2 = np.copy(
#             point_numbers[:FIBONACCI[num_digits+2]])
#         point_numbers_2 = scramble_numbers(point_numbers_2, 30)
...
# point_numbers_2 = np.copy(
#     point_numbers[FIBONACCI[num_digits+3]:FIBONACCI[num_digits+4]])
# point_numbers_2 = scramble_numbers(point_numbers_2, 30)
# point_set = convert_integer_points_to_decimals(point_numbers_2)
# plot_points(point_set, 0, 0)


# point_numbers_2 = np.copy(point_numbers[:FIBONACCI[num_digits+2]])
# point_numbers_2 = scramble_numbers(point_numbers_2, 30)
# point_set = convert_integer_points_to_decimals(point_numbers_2)
# plot_points(point_set, 0, 0)
...
# scramble_numbers_m_digits(30, numbers)
# print()
# scramble_numbers_m_digits(30, numbers)
# scramble_numbers_m_digits_2(30, numbers)
