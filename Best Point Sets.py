...
def best_l2_disc(num_digits):
    x_digits = no_consecutive_ones(num_digits)
    x_digits = x_digits[1:]
    x_values = convert_to_decimal(x_digits)
    best_disc = 10.0

    for perms in list(itertools.permutations(range(fibonacci(num_digits+2)-1))):
        points = np.append(x_values, x_values[[perms]], axis=1)
        points = np.append(points, np.array([[0, 0]]), axis=0)
        disc = version_3_l2_star_discrepancy(points)
        if disc <= best_disc:
            best_disc = disc
            best_points = np.copy(points)
            best_digits = x_digits[[perms]]
    # plot_points(best_points, num_digits, num_digits)
    print(best_disc)
    print(best_points)
    print(best_digits)
    print(x_digits)
...
m = 3
good_points_3 = best_l2_disc(m)
# plot_points(good_points_3, 4, 4)
print(version_3_l2_star_discrepancy(good_points_3))

print(best_l2_disc(m))
