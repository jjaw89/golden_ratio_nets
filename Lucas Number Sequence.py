...
def cyclic_shift(num_digits, shift):
    numbers = no_consecutive_ones_binary_flipped(num_digits)
    for _ in range(shift):
        first_digits = numbers % 2
        numbers = numbers // 2
        numbers += first_digits * 2 ** num_digits
    return numbers


def scramble_numbers_m_digits_cp_swap_prob_1(m, numbers_gpu):

    random_zeros_and_ones_gpu = cp.ones((FIBONACCI[m],), dtype='uint32')
    random_zeros_and_ones_gpu *= 2**(m-1)
    permutation_gpu = cp.copy(
        ALL_THIRTY_TWO_DIGIT_NUMBERS_GPU[:FIBONACCI[m+3]])
    permutation_gpu[:FIBONACCI[m]] += random_zeros_and_ones_gpu
...
def scramble_non_zero_digits_no_back_tracking_cp_swap_prob_1(numbers, num_digits):
    numbers = np.array(numbers, dtype='uint32')
    numbers_gpu = cp.asarray(numbers)
    for m in range(1, num_digits+1):
        numbers_gpu = scramble_numbers_m_digits_cp_swap_prob_1(
            m, numbers_gpu)
    numbers = cp.asnumpy(numbers_gpu)
    return numbers


def points_numbers_scrambled_swap_prob_1(num_digits, x_num_swaps, y_num_swaps):
    x = no_consecutive_ones_binary_flipped(num_digits)
    for _ in range(x_num_swaps):
        x = scramble_non_zero_digits_no_back_tracking_cp_swap_prob_1(
            x, num_digits)
    y = no_consecutive_ones_binary_flipped(num_digits)
    for _ in range(y_num_swaps):
        y = scramble_non_zero_digits_no_back_tracking_cp_swap_prob_1(
            y, num_digits)

    point_numbers = np.array(
        [[x[n], y[n]] for n in range(FIBONACCI[num_digits+2])], dtype='int32')
    return point_numbers
...
#     points_numbers = points_numbers_scrambled_swap_prob_1(num_digits, 0, n)
#     if np.array_equal(points_numbers[:, 0], points_numbers[:, 1]):
#         is_true = True
#         break

# print(convert_numbers_to_digits(
#     no_consecutive_ones_binary_flipped(num_digits), num_digits))
# print(convert_numbers_to_digits(
#     no_consecutive_ones_binary_2(num_digits), num_digits))

# print(f'{is_true}: {n}')
# for k in range(n):
#     points_numbers = points_numbers_scrambled_swap_prob_1(num_digits, 0, k)
#     plot_points(convert_integer_points_to_decimals(points_numbers), 0, 0, 10)


# for n in range(1, 10):

#     disc = (GOLDEN_RATIO-1)/(4*GOLDEN_RATIO**n)*n + (1/GOLDEN_RATIO **
#                                                      n)*(5/4+1/GOLDEN_RATIO) - 1/(4*GOLDEN_RATIO**(2*n))
#     print(f'{n} , {disc}')
...
def lucas_hammersley(num_digits):

    # print(GOLDEN_RATIO**num_digits)
    y_numbers = no_consecutive_ones_binary_2(num_digits+1)
    # print(convert_numbers_to_digits(y_numbers, 5))
    # print()
    y_numbers = np.append(
        y_numbers[:FIBONACCI[num_digits+1]], y_numbers[-FIBONACCI[num_digits-1]:], axis=0)
    # print(convert_numbers_to_digits(y_numbers, 6))
    # print(y_numbers.shape[0])
    y_values = convert_integers_to_decimals(y_numbers)
    x_values = np.sort(y_values)
    points = np.array([[x_values[n], y_values[n]]
                       for n in range(y_values.shape[0])], dtype='float')
    # print(points)
    return points


# print(lucas_hammersley(3).shape[0])
# for num_digits in range(15, 25):
#     plot_points(lucas_hammersley(num_digits), 0, 0, 1)

# plot_points(lucas_hammersley(5), 2, 1, 10)
vector_results_l2 = np.zeros((25, 10))
vector_results_star = np.zeros((25, 10))
for n in range(1):

    for num_digits in range(3, 22):  # 22
        point_set = lucas_hammersley(num_digits)
        vector_results_l2[num_digits, n] = (
            (version_3_l2_star_discrepancy(point_set))**.5).real
        # vector_results_star[num_digits,
        #                     n] += [star_discrepancy_parallel(point_set)]

        pd.DataFrame(vector_results_l2).to_csv(
            'L2 star Discrepancy Scrambled Sequence.csv')
        pd.DataFrame(vector_results_star).to_csv(
            'Star Discrepancy Scrambled Sequence.csv')
        print(f"n : {n} , num digits : {num_digits}")
...
# # print(convert_numbers_to_digits(y_numbers, 5))
# print()
# bool = np.invert(np.multiply(y_numbers % 2 == 1, y_numbers //
#                              2 ** (num_digits-1) == 1))
# # print(bool)
# print(convert_numbers_to_digits(y_numbers[bool], 4))
# y_values = convert_integers_to_decimals(y_numbers[bool])
...
# # print(convert_numbers_to_digits(y_numbers, 5))
# print()
# bool = np.invert(np.multiply(x_numbers % 2 == 1, x_numbers //
#                              2 ** (num_digits-1) == 1))
# # print(bool)
# print(convert_numbers_to_digits(x_numbers[bool], 5))
# x_values = convert_integers_to_decimals(x_numbers[bool])
# points = np.array([[y_values[n], y_values[n]]
#                    for n in range(y_values.shape[0])], dtype='float')
# # print(points)
# plot_points(points, 2, 0, 10)


# print(convert_numbers_to_digits(no_consecutive_ones_binary_2(4), 4))

# for num_digits in range(1, 25):

#     points = lucas_hammersley(num_digits)
...
#     # print(f'{num_digits} : N Time {total_time}')

#     # print(num_digits)
#     if result[0, 0] > result[1, 0]:
#         print(
#             f'{num_digits}, {points.shape[0]} , c, {result[0,0]}, {result[0,1]} , {result[0,2]}')
#     else:
#         print(
#             f'{num_digits}, {points.shape[0]} , o, {result[0,0]}, {result[0,1]} , {result[0,2]}')
#     # print(result)
