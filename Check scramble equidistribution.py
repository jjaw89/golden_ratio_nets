...
def partition_numbers_2d_binary(digits_to_partition, x_partition_digits, y_partition_digits):
    last_digit_x = np.zeros((FIBONACCI[x_partition_digits+2]), dtype='int64')
    last_digit_x[1] = 1
    ...
    last_digit_y = np.zeros((FIBONACCI[y_partition_digits+2]), dtype='int64')
    last_digit_y[1] = 1
    ...
    return np.array([[FIBONACCI[digits_to_partition+2-x_partition_digits-y_partition_digits-r-c] for r in last_digit_y] for c in last_digit_x], dtype='int64')
...
def find_t_value(point_numbers, num_digits):
    truth = False
    for s in range(1, num_digits+1):
        for k in range(s+1):
            counting_matrix = check_equidistribution_njit(
                point_numbers[:, 0], point_numbers[:, 1], num_digits, k, s-k)
            # print(f'{k},{s-k}')
            # print(counting_matrix)
...
# print(check_scramble_equidistribution(3))

point_numbers = generate_hammersley_numbers(4)
scrambled_point_numbers = scramble_non_zero_digits_back_tracking(
    np.copy(point_numbers))
scrambled_points = convert_to_points_and_scramble_tail_end(
    scrambled_point_numbers)
for _ in range(30000):
    scrambled_point_numbers = scramble_non_zero_digits_back_tracking(
        np.copy(point_numbers))
    scrambled_points_temp = convert_to_points_and_scramble_tail_end(
        scrambled_point_numbers)
    scrambled_points = np.append(
        scrambled_points, scrambled_points_temp, axis=0)

plot_points(scrambled_points, 0, 0)
...
#         scrambled_point_numbers = scramble_non_zero_digits_back_tracking(
#             np.copy(point_numbers))
#         point_set = convert_to_points_and_scramble_tail_end(
#             scrambled_point_numbers)
