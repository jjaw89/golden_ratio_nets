...
# for _ in range(13):  # 26 for starting with 2
#     start_time = time.perf_counter()
...
# for y_part in range(1, num_digits):
#     plot_points(points, num_digits-y_part, y_part, 10)


# print(convert_point_numbers_to_digits(generate_hammersley_numbers(5), 5))

# plot_points(hammersley_points(5), 3, 3, 10)


def plot_grid(x_partition, ypartition):
    plt.figure(figsize=(12, 12))
    ax = plt.gca()
    ax.set_aspect(1)
    # plt.plot(x, y, '.', markersize=size)
...
def number_line_node(num_digits):
    part = partition(num_digits)
    numbers = no_consecutive_ones_binary_flipped(num_digits)
    digits = convert_numbers_to_digits(numbers, num_digits)
    decimals = []
    for digit in digits:
        decimal = '.'
        for d in digit:
    ...
    lower_label = '\\foreach \\x/\\xtext in {0/0,'

    for n in range(1, FIBONACCI[num_digits+2]):
        lower_label += str(part[n])
        lower_label += "/"
        lower_label += decimals[n]
        lower_label += ","

    lower_label += "1/1}"
    print(lower_label)
    print("\t\\draw[thick] (\\x,.5pt)  (\\x,-.5pt) node[below] {\\xtext};")

    for n in range(FIBONACCI[num_digits+2]):
        print(f'\draw[[-), thick, black] ({part[n]},0) -- ({part[n+1]},0);')
...
# plot_points(points, 3, 3, 20)

# number_line_node(5)

# k = 1+8
# point_numbers = np.array([[8, 8], [k, k], [10, 10]], dtype='int64')
# points = convert_integer_points_to_decimals(point_numbers)
# plot_points(points, 1, 1, 30)

# point_numbers = np.array([[8, 8], [k, 10], [10, k]], dtype='int64')
# points = convert_integer_points_to_decimals(point_numbers)
# plot_points(points, 1, 1, 30)
...
# x = 10  # 2
# y = 1+4  # 5
# point_numbers_initial = np.array([[k, k], [m, m], [n, n]], dtype='int64')
# points_initial = convert_integer_points_to_decimals(point_numbers_initial)
# point_numbers_new = np.array([[x, 0], [y, 0]], dtype='int64')
# points_new = convert_integer_points_to_decimals(point_numbers_new)


# x_initial = points_initial[:, 0]
# y_initial = points_initial[:, 1]
# x_new = points_new[:, 0]
# y_new = points_new[:, 1]
# plt.figure(figsize=(12, 12))
# ax = plt.gca()
# ax.set_aspect(1)
# plt.plot(x_initial, y_initial, '.', markersize=size, c=c_initial)
# plt.plot(x_new, y_new, '.', markersize=size, c=c_new)
# plt.grid()
# plt.xticks(ticks=partition(x_partition), labels=[])
# plt.yticks(ticks=partition(y_partition), labels=[])
# plt.xlim((0, 1))
# plt.ylim((0, 1))
# plt.grid()
# plt.show()

# point_numbers_new = np.array([[x, 1], [y, 0]], dtype='int64')
# points_new = convert_integer_points_to_decimals(point_numbers_new)
...
# point_numbers_initial = np.array([[k, k], [n, m], [m, n]], dtype='int64')
# points_initial = convert_integer_points_to_decimals(point_numbers_initial)
# point_numbers_new = np.array([[x, 0], [y, 0]], dtype='int64')
# points_new = convert_integer_points_to_decimals(point_numbers_new)
...
# point_numbers_new = np.array([[x, 0], [y, 1]], dtype='int64')
# points_new = convert_integer_points_to_decimals(point_numbers_new)
...
def plot_points_equidistribution(points, x_partition, ypartition, size):
    x = points[:, 0]
    y = points[:, 1]
    plt.figure(figsize=(12, 12))
    ax = plt.gca()
...
for n in range(1, 4):
    plot_points(convert_integer_points_to_decimals(
        generate_hammersley_numbers(m)), n, 4-n, 15)
for n in range(1, 5):
    plot_points(convert_integer_points_to_decimals(
        generate_hammersley_numbers(m)), n, 5-n, 15)
for n in range(1, 6):
    plot_points(convert_integer_points_to_decimals(
        generate_hammersley_numbers(m)), n, 6-n, 15)
