...
def generate_sequence_new(num_digits, pattern):
    ...
    for current_digit in range(3, num_digits+1):
        ...
        second_chunk = sequence_numbers[FIBONACCI[current_digit-2]:FIBONACCI[current_digit-1]]
        third_chunk = sequence_numbers[FIBONACCI[current_digit-1]:FIBONACCI[current_digit]]
...
# num_digits = 8
# x_coord = generate_sequence_new(num_digits, [0])
# y_coord = generate_sequence_new(num_digits, [1])
# point_numbers = np.array([[x_coord[n], y_coord[n]]
#                          for n in range(FIBONACCI[num_digits+2])], dtype='int64')
# print(convert_point_numbers_to_digits(point_numbers, num_digits))


# num_digits = 8
# digits = convert_point_numbers_to_digits(
#     starting_base_sequence(num_digits), num_digits)
# numbers = starting_base_sequence(num_digits)
# for n in range(digits.shape[0]):
#     print(f'{digits[n][0]} ,{numbers[n][0]}, {digits[n][1]}, {numbers[n][1]}')

# sequence_y = np.array([0, 2, 1, 5, 4, 8, 10, 9, 16, 21, 18, 17, 20, 32, 34, 37, 40, 41, 33, 42, 36, 64, 66, 81, 68, 85, 72, 82, 73, 65,
#                       72, 74, 69, 84, 128, 130, 145, 136, 169, 132, 138, 149, 160, 161, 146, 164, 165, 129, 162, 144, 137, 168, 133, 170, 148], dtype='int64')

# sequence_y = np.array([0, 2, 1, 5, 4, 8, 10, 9, 16, 21, 18, 17, 20, 32, 34, 37, 40, 41, 33, 42, 36, 64, 66, 81, 68, 85, 72, 82, 73, 65,
#                       72, 74, 69, 84, 160, 168, 164, 162, 170, 161, 169, 165, 144, 148, 146, 145, 149, 128, 136, 132, 130, 138, 129, 137, 133], dtype='int64')
# sequence_y = np.array([0, 2, 1, 5, 4, 8, 10, 9, 16, 20, 18, 17, 21, 40, 36, 42, 41, 37, 32, 36, 34, 64, 66, 81, 68, 85, 72, 82, 73, 65,
#                       72, 74, 69, 84, 160, 168, 164, 162, 170, 161, 169, 165, 144, 148, 146, 145, 149, 128, 136, 132, 130, 138, 129, 137, 133], dtype='int64')
# numbers[:, 1] = sequence_y
# points = convert_integer_points_to_decimals(numbers)
# m = 9
# # # plot_points(points[FIBONACCI[m]:], 0, 0, 10)
# # # # plot_points(points[FIBONACCI[m]: FIBONACCI[m]+FIBONACCI[m-3]], 0, 0, 10)
# # plot_points(points[FIBONACCI[m]: FIBONACCI[m]+FIBONACCI[m-2]], 1, 1, 10)
# # # # plot_points(points[FIBONACCI[m]+FIBONACCI[m-3]            : FIBONACCI[m]+FIBONACCI[m-3]+FIBONACCI[m-4]], 0, 0, 10)
# # # plot_points(points[FIBONACCI[m]+FIBONACCI[m-3]+FIBONACCI[m-4]            : FIBONACCI[m]+2*FIBONACCI[m-3]+FIBONACCI[m-4]], 0, 0, 10)

# # plot_points(points[FIBONACCI[m]:], 1, 2, 10)
# # plot_points(points, 1, 2, 10)
# # sequence_y = np.array([0,2,1,5,4,8,10,9,16,20,18,17,21,40,36,42,41,37,32,36,34,64,66,81,68,85,72,82,73,65,72,74,69,84,160,168,164,162,170,161,169,165,144,148,146,145,149,128,136,132,130,138,129,137,133], dtype='int64')

# points = hammersley_points(3)

# plot_points(points, 4, 1, 10)
# plot_points(points, 5, 5, 1)
# num_digits = 8
# van_der_corput = no_consecutive_ones_binary_2(num_digits)
# y_coord = np.array([0,  1,  2,  5,  4,  8, 10,  9, 17, 16,
#                    20, 18, 21, 42, 40, 37, 36, 34, 33, 41, 32, 85, 82, 72, 80, 81, 66, 84, 74, 68, 69, 65, 73, 64], dtype='int64')
# point_numbers = np.zeros((FIBONACCI[num_digits+2], 2), dtype='int64')
# point_numbers[:, 0] = van_der_corput
# point_numbers[:FIBONACCI[num_digits+1], 1] = y_coord
# point_values = convert_integer_points_to_decimals(point_numbers)


# plot_points(point_values[:FIBONACCI[8]],
#             0, 0, 10)
# # for x_part in range(1, 4):

# plot_points(point_values[FIBONACCI[8]:FIBONACCI[8]+8],
#             0, 0, 10)

# plot_points(point_values[FIBONACCI[8]+8:FIBONACCI[8]+8+5],
#             0, 0, 10)


# for x_part in range(1, 6):
#     plot_points(point_values[0:1],
#                 x_part, num_digits-2-x_part, 10)

# for x_part in range(1, 5):
#     plot_points(point_values[0:1],
#                 x_part, num_digits-3-x_part, 10)

# for x_part in range(1, 4):
#     plot_points(point_values[0:1],
#                 x_part, num_digits-4-x_part, 10)
# for x_part in range(1, 3):
#     plot_points(point_values[0:1],
#                 x_part, 3-x_part, 10)

# for x_part in range(1, 2):
#     plot_points(point_values[0:1],
#                 x_part, 2-x_part, 10)
...
# for _ in range(20):  # 26 for starting with 2
#     start_time = time.perf_counter()
...
# n = 10
# point_values = convert_integer_points_to_decimals(point_numbers[:FIBONACCI[n]])
# plot_points(point_values[FIBONACCI[n-1]:FIBONACCI[n]], 0, 0, 10)
# print(convert_point_numbers_to_digits(
#     point_numbers[FIBONACCI[n-1]:FIBONACCI[n]], 9))

# num_digits = 6
# numbers = no_consecutive_ones_binary_2(num_digits)
# take_order = np.zeros_like(numbers, dtype='int64')
# for k in range(numbers.shape[0]):
#     take_order[k] = k*FIBONACCI[num_digits-1] % FIBONACCI[num_digits+2]

# print(take_order)
# new_numbers = numbers[take_order]
