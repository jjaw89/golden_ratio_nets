...
#     sum = cp.array([cp.sum(cp.minimum(cuda_points_x, cp.roll(cuda_points_x, n))
#                            * cp.minimum(cuda_points_y, cp.roll(cuda_points_y, n))-1/9) for n in range(1, cuda_points_x.shape[0])])
#     return cp.sum(sum)
...
#     start_time = time.perf_counter()
#     value_1 = l2_star_discrepancy_cuda_first_part(points)
#     end_time = time.perf_counter()
#     time_difference_1 = end_time-start_time

#     start_time = time.perf_counter()
#     value_2 = first_part_version_2(points)
#     end_time = time.perf_counter()
#     time_difference_2 = end_time-start_time
#     print(value_1-value_2)
#     print(f'{num_digits}: {time_difference_1} : {time_difference_2}')

# for num_digits in range(1, 26):
#     points = convert_integer_points_to_decimals(
#         generate_hammersley_numbers(num_digits))

#     start_time = time.perf_counter()
#     value_1 = l2_star_discrepancy_cuda_second_part(points)
#     end_time = time.perf_counter()
#     time_difference_1 = end_time-start_time
#     print(f'{num_digits}: {time_difference_1} ')


# for num_digits in range(25, 26):
#     points = convert_integer_points_to_decimals(
#         generate_hammersley_numbers(num_digits))
#     start_time = time.perf_counter()
#     value_2 = second_part_version_3(points)
#     end_time = time.perf_counter()
#     time_difference_2 = end_time-start_time
#     print(f'{num_digits}:{time_difference_2}')

# path = Path("~").expanduser()
# path.mkdir(parents=True, exist_ok=True)
# # L2_star_discrepancy = np.zeros(35, dtype='float')
...
#     start_time = time.perf_counter()
#     L2_star_discrepancy[num_digits] = l2_star_discrepancy_cuda(points)
#     end_time = time.perf_counter()
#     time_difference_1 = end_time-start_time

#     print(f'{num_digits} : {time_difference_1}')
# np.save(path/'L2_star_discrepancy.npy', L2_star_discrepancy)

points = convert_integer_points_to_decimals(
    generate_hammersley_numbers(30))
print(star_discrepancy_largest_rest(points))

# test_points = np.array([[1, 2], [2, 3], [4, 5]])
# print(np.sum(np.product(test_points, axis=1)-1))
# print(L2_star_discrepancy)
# l2_star_discrepancy = np.load(path/f'L2_star_discrepancy.npy')
# # print(l2_star_discrepancy)

# for n in range(l2_star_discrepancy.shape[0]):
#     print(f'{l2_star_discrepancy[n]},')

# for num_digits in range(25):
#     points = convert_integer_points_to_decimals(
#         generate_hammersley_numbers(num_digits))
#     print(
#         1/FIBONACCI[num_digits+2]*l2_star_discrepancy_cuda_third_part(points) / num_digits)
...
#     sum_1 = 0.
#     for m in range(FIBONACCI[num_digits+2]):
#         sum_1 += (1-points[m, 0]**2)*(1-points[m, 1]**2)
#     sum_1 /= 2*points.shape[0]

#     sum_2 = 0.
#     for m in range(FIBONACCI[num_digits+2]):
#         for n in range(FIBONACCI[num_digits+2]):
#             sum_2 += min(1-points[m, 0], 1-points[n, 0]) * \
#                 min(1-points[m, 1], 1-points[n, 1])
#     sum_2 /= points.shape[0]**2
#     disc = 1/9 - sum_1+sum_2

#     print(f'{disc},{l2_star_discrepancy_cuda(points)}')
