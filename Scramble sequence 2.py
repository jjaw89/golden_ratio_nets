...
def generate_random_sequence(num_digits):
    ...
    for m in range(1, num_digits):
        ...
        # points = convert_integer_points_to_decimals(point_numbers_2)
...
@njit()
def generate_random_sequence_bad_first_point(num_digits):

    first_point = np.array([[0, 0]], dtype='int64')
    previous_digit = 0
    for k in range(32):
        if previous_digit == 0:
            previous_digit = random.randint(0, 1)
            first_point[0, 0] += previous_digit * 2 ** k
        else:
            previous_digit = 0
    previous_digit = 0
    for k in range(32):
        if previous_digit == 0:
            previous_digit = random.randint(0, 1)
            first_point[0, 1] += previous_digit * 2 ** k
        else:
            previous_digit = 0
    # first_point = np.array([[random_no_consecutive_ones_number(
    #     32), random_no_consecutive_ones_number(32)]], dtype='int64')
    # print(first_point)
    # print(convert_point_numbers_to_digits(first_point, 8))
    if num_digits == 0:
        return first_point
    ...
    # plot_points(points_1, 1, 1)
    # plot_points(points_2, 2, 2)
    # print(convert_point_numbers_to_digits(point_numbers_2, 6))
    # num_digits = 5
    # x_numbers = no_consecutive_ones_binary_flipped(num_digits)
    # y_numbers = no_consecutive_ones_binary_2(num_digits)
    # point_numbers = np.array([[x_numbers[n], y_numbers[n]]
    #                          for n in range(x_numbers.shape[0])], dtype='int64')
    # print(point_numbers)
    # print(point_numbers_2)
    for m in range(1, num_digits):
        # print(point_numbers_2.shape)
        point_numbers_2 = extend_points_randomized(point_numbers_2, m)
        # points = convert_integer_points_to_decimals(point_numbers_2)
        # plot_points(points, 0, 0)
...
@ njit(parallel=True)
def place_zero_equal_prob(num_points):
    numbers = np.zeros((num_points, 2), dtype='int64')
    for n in prange(num_points):
        previous_digit = 0
        for k in range(32):
            if previous_digit == 0:
                previous_digit = random.randint(0, 1)
                numbers[n, 0] += previous_digit * 2 ** k
            else:
                previous_digit = 0
        previous_digit = 0
        for k in range(32):
            if previous_digit == 0:
                previous_digit = random.randint(0, 1)
                numbers[n, 1] += previous_digit * 2 ** k
            else:
                previous_digit = 0
...
# points = convert_integer_points_to_decimals(generate_random_sequence(20))

# plot_points_size_1(points, 0, 0)
# print(points.shape)

# points = np.random.rand(points.shape[0], 2)
# plot_points_size_1(points, 0, 0)


# vector_results_l2 = np.zeros((25, 11))
# vector_results_star = np.zeros((25, 11))

# for n in range(3, 22):
#     vector_results_l2[n, 0] = FIBONACCI[n]
#     vector_results_star[n, 0] = FIBONACCI[n]

# for n in range(1, 11):
#     point_numbers = generate_random_sequence(21)
...
for num_digits in range(1, 20):
    ...
    start_time = time.perf_counter()
    point_numbers = generate_random_sequence_bad_first_point(num_digits)
    ...
    for n in range(num_repeats):
        ...
        points_temp = convert_integer_points_to_decimals(point_numbers)
...
# # for n in range(5, 26):
# #     print(n-2)
# plot_points_size_1(points[FIBONACCI[num_digits+1]:], 0, 0)
# print(points[FIBONACCI[num_digits+1]:].shape)
# plot_points_size_1(points[:FIBONACCI[num_digits]], 0, 0)
# print(points[:FIBONACCI[num_digits]].shape)


# place_zero_equal_prob(3)
# start_time = time.perf_counter()
# points = place_zero_equal_prob(250000)
# # print(points)
# end_time = time.perf_counter() - start_time
# plot_points_size_1(points, 0, 0)
# print(end_time)

# print(convert_point_numbers_to_digits(place_zero_equal_prob(10), 6))
